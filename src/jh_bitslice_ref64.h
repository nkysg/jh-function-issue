...
void RoundFunction(hashState* state, uint64 roundnumber);  /*the round function of E8, in bitslice form*/
void E8(hashState *state);   /*The bijective function E8, in bitslice form */
void F8(hashState *state);  /*The compression function F8, in bitslice form*/
...
void RoundFunction(hashState* state, uint64 roundnumber) {
      uint64 i,j,temp0;

      /*Sbox and MDS layer*/
      for (i = 0; i < 2; i++) {
            Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((uint64*)E8_bitslice_roundconstant[roundnumber])[i] );
            Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((uint64*)E8_bitslice_roundconstant[roundnumber])[i+2] );
      ...
      {
            case 0:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 2; i++) SWAP1(state->x[j][i]);
                  break;
            case 1:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 2; i++) SWAP2(state->x[j][i]);
                  break;
            case 2:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 2; i++) SWAP4(state->x[j][i]);
                  break;
            case 3:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 2; i++) SWAP8(state->x[j][i]);
                  break;
            case 4:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 2; i++) SWAP16(state->x[j][i]);
                  break;
            case 5:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 2; i++) SWAP32(state->x[j][i]);
                  break;
            case 6:
                  for (j = 1; j < 8; j = j+2) {
                        temp0 = state->x[j][0]; state->x[j][0] = state->x[j][1]; state->x[j][1] = temp0;
                  }
                  break;
      }
}
